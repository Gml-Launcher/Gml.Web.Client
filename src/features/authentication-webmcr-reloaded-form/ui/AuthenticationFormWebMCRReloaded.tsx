...
import { HREF_GET_WEBMCR_RELOADED_AUTH_PHP } from '@/shared/constants';

interface SignInFormProps extends React.HTMLAttributes<HTMLDivElement> {
  onOpenChange: (open: boolean) => void;
}

export function AuthenticationFormWebMCRReloaded({
  className,
  onOpenChange,
  ...props
}: SignInFormProps) {
  ...
  const form = useForm<IntegrationFormSchemaType>({
    values: {
      endpoint:
        integration?.authType === AuthenticationType.AUTHENTICATION_TYPE_WEB_MCR_RELOADED
          ? String(integration.endpoint)
          : '',
      authType:
        integration?.authType === AuthenticationType.AUTHENTICATION_TYPE_WEB_MCR_RELOADED
          ? integration.authType
          : AuthenticationType.AUTHENTICATION_TYPE_WEB_MCR_RELOADED,
    },

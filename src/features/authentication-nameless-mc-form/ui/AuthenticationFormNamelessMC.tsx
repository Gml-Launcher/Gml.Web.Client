...
import { HREF_GET_NAMELESS_MC_AUTH_PHP } from '@/shared/constants';

interface SignInFormProps extends React.HTMLAttributes<HTMLDivElement> {
  onOpenChange: (open: boolean) => void;
}

export function AuthenticationFormNamelessMC({
  className,
  onOpenChange,
  ...props
}: SignInFormProps) {
  ...
  const form = useForm<IntegrationFormSchemaType>({
    values: {
      endpoint:
        integration?.authType === AuthenticationType.AUTHENTICATION_TYPE_NAMELESS_MC
          ? String(integration.endpoint)
          : '',
      authType:
        integration?.authType === AuthenticationType.AUTHENTICATION_TYPE_NAMELESS_MC
          ? integration.authType
          : AuthenticationType.AUTHENTICATION_TYPE_NAMELESS_MC,
    },
            ...
            {form.formState.errors.endpoint ? (
              ...
              <FormDescription>
                ...
                <a
                  ...
                  href={HREF_GET_NAMELESS_MC_AUTH_PHP}
                  target="_blank"
                  download="auth.php"
                >
                  Скачайте по ссылке
